...
export class IpcManager {
    static initialize() {
        ...
        this.handleLmdbControls();
    }

    private static handleLmdbControls() {
        ipcMain.handle("lmdb:get", async (_,  dbName: string,key: string) => {
            return lmdbService.get(dbName, key);
        });

        ipcMain.handle("lmdb:put", async (_, dbName: string, key: string, value: any) => {
            return lmdbService.put(dbName,key, value);
        });

        ipcMain.handle("lmdb:remove", async (_, dbName: string, key: string) => {
            return lmdbService.remove(dbName, key);
        });

        ipcMain.handle('lmdb:getDbPath', () => {
            return lmdbService.dbPath; // You'll need to make dbPath public or add a getter
        });

        ipcMain.handle("lmdb:listKeys", async (_, dbName: string, prefix?: string) => {
            return lmdbService.listKeys(dbName, prefix);
        });
