...
export class IpcManager {
  ...
  private static handleLmdbControls() {
    ipcMain.handle("lmdb:get", async (_, key: string) => {
      return lmdbService.get(key);
    });

    ipcMain.handle("lmdb:put", async (_, key: string, value: any) => {
      return lmdbService.put(key, value);
    });

    ipcMain.handle("lmdb:remove", async (_, key: string) => {
      return lmdbService.remove(key);
    });
