...
Built with **React 19**, **TypeScript**, **Redux Toolkit**, and packaged as an **Electron** desktop app via [electron-vite](https://github.com/electron-vite/electron-vite-react). UI styling uses a custom CSS design system.
...
The UI expects the SkellyCam Python server running on `localhost:53117` (default). Start the server first:
...
**Recording Browser** — lists all recordings from `~/skellycam_data/recordings/`, sorted newest-first. Each entry shows:
...
- `GET /skellycam/playback/recordings` — list all recordings with metadata (size, frames, duration, fps)
- `POST /skellycam/playback/load` — load a recording folder
- `GET /skellycam/playback/video/{video_id}` — stream a video file (supports HTTP range requests for seeking)
- `GET /skellycam/playback/timestamps/{video_id}` — get timestamp CSV info

**WebSocket** — real-time binary frame streaming at `ws://localhost:53117/skellycam/websocket/connect`. Delivers synchronized multi-frame payloads (one JPEG per camera per frame event) plus JSON log messages.
