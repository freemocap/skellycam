...
<AiGeneratedBanner humanCurated humanNotes="Should add something telling users to please be patient on PR's which could take a long time for us to review. Should add something that references Nadia Egbhal's Working in Public about developer attention being the main limited resource. ALso should link to the Hot/hard/soft loop section of the Architecture to clue potential contributors in on that distinction. Specifically, letting them know that contributions that touch the hot/hard loops will be given much higher scrutiny than other contributions. For first time contibutors, consider making contact in the server or citing credentials if contributing that touch the sensitive parts of the app"/>
