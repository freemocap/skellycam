...
<AiGeneratedBanner humanCurated humanNotes="Should add a note that the frontend/backend processes are independent, so e.g. if they are only working on backend code they can use the standard app installer to run the frontend and turn off 'auto-connect' and then run their backend code from source (and vice versa for frontend-only work) />

# Development

## Install from Source

This section is for developers and contributors who want to run SkellyCam from the source code. If you just want to use SkellyCam, download the installer from the [Download page](/download) instead.
