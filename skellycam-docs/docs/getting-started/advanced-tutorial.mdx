...
[//]: # (TODO - add a note saying "set to `~/freemocap_data/recording_session` in order to save skellycam recordings directly to the directory where freemocap v1 will see them automatically)
All SkellyCam data is stored under `~/skellycam_data/recordings/` by default.

### Naming convention

[//]: # ( TODO - lets make the ISO8601 thing link to somethng explaining what it is. A lot of folks have their own personal fav timestamp format but they SHOULD just follow the ISO8601 standard lol)
...
[//]: # (TODO - include a link to something explainig why perf_counter_ns is preferred for high precision applications. Maybe a brief thing to say that we shouldnt trust the ns scale values, but this format helps us avoid floating point error)
...
[//]: # (TODO - Note the use of FOURCC codes is pretty basic atm. link to the issue #105 - https://github.com/freemocap/skellycam/issues/105)
