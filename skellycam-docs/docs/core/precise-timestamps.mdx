...
import { AiGeneratedBanner, CoreFeatureHeader, LinkedIssues } from '@freemocap/skellydocs';
...
<details>
<summary>Does it really matter though?</summary>
...
- **Monotonic** — the clock never goes backward, even during [NTP](https://en.wikipedia.org/wiki/Network_Time_Protocol) adjustments (NTP is the protocol that keeps your system clock synchronized with internet time servers).
- **Shared across processes** — all processes on the same machine reference the same monotonic clock

The [TimebaseMapping](https://github.com/freemocap/skellycam/blob/development/skellycam/core/camera/config/timebase_mapping.py) class is the primary tool SkellyCam uses to ensure a consistent timebase across all camera processes.
...
SkellyCam captures high-resolution [`perf_counter_ns`](https://docs.python.org/3/library/time.html#time.perf_counter_ns) timestamps at multiple stages of each frame's lifecycle (defined in the [frame metadata structures](https://github.com/freemocap/skellycam/tree/development/skellycam/core/camera/internal/frame) in the source code):
