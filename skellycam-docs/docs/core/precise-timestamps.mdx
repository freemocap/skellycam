...
[//]: # (TODO - should add a side-note block  like "Does it really matter? and say a bit about "Does this level of temporal precision really matter for freemocap output? The answer is an empircal question and depends on the application. However, the fact remains that triangulation accuracy is directly coupled to temporal precision between the multi-view samples, so in skellycam we treat synchronization precision as an 'aspirational goal' where the goal is perfect synchronization )
...
[//]: # (TODO - &#40;see note elsewhere that we shouldnt trust ns in python, but perf_counter_ns helps avoid floating point error&#41; Also what is NTP ADjustment? SHould reference the TimebaseMapping class in skellycam, as thats the main tool we use to ensure consistent timebase across our processes )
...
[//]: # (TODO - reference skellycam code defintiions of the timestamps and whatnot)
...
[//]: # (TODO - reference Issue 99 to clean up the tiemstamp output into a tidy format: https://github.com/freemocap/skellycam/issues/99)
