...
export function getAppInstallInstructions(
  ...
  if (os === 'macos') {
    return [
      {
        text: '<strong>.dmg</strong> \u2014 Open the disk image and drag SkellyCam into Applications. On first launch, right-click the app and select <strong>Open</strong> to bypass Gatekeeper.',
      },
      {
  ...
  if (os === 'linux') {
    ...
    const ai = `skellycam_${version}_${label}.AppImage`;
    const deb = `skellycam_${version}_${label}.deb`;
...
export function getServerInstallInstructions(
  ...
  const srv =
    os === 'windows'
      ? `skellycam_server_${version}_${label}.exe`
      : `skellycam_server_${version}_${label}`;
...
export function getTerminalInstallBlocks(
  ...
  if (os === 'linux') {
    const ai = `skellycam_${version}_${label}.AppImage`;
    const deb = `skellycam_${version}_${label}.deb`;
    const srv = `skellycam_server_${version}_${label}`;

    blocks.push({
      codeLines: [
        { type: 'comment', content: '# App Installer \u2014 AppImage (any distro, no root)' },
        { type: 'prompt', content: `curl -fSL -o skellycam.AppImage "${baseUrl}/${ai}"`, promptChar: '$' },
        { type: 'prompt', content: 'chmod +x skellycam.AppImage', promptChar: '$' },
        { type: 'prompt', content: './skellycam.AppImage', promptChar: '$' },
      ],
    });
    blocks.push({
      codeLines: [
        { type: 'comment', content: '# App Installer \u2014 .deb (Debian/Ubuntu)' },
        { type: 'prompt', content: `curl -fSL -o skellycam.deb "${baseUrl}/${deb}"`, promptChar: '$' },
        { type: 'prompt', content: 'sudo apt install ./skellycam.deb', promptChar: '$' },
      ],
    });
    blocks.push({
      codeLines: [
        { type: 'comment', content: '# Backend Server (headless / Raspberry Pi)' },
        { type: 'prompt', content: `curl -fSL -o skellycam_server "${baseUrl}/${srv}"`, promptChar: '$' },
        { type: 'prompt', content: 'chmod +x skellycam_server', promptChar: '$' },
        { type: 'prompt', content: './skellycam_server', promptChar: '$' },
      ],
    });
  }

  if (os === 'macos') {
    const dmg = `skellycam_${version}_${label}.dmg`;
    const srv = `skellycam_server_${version}_${label}`;

    blocks.push({
      codeLines: [
        { type: 'comment', content: '# App Installer' },
        { type: 'prompt', content: `curl -fSL -o skellycam.dmg "${baseUrl}/${dmg}"`, promptChar: '$' },
        { type: 'prompt', content: 'open skellycam.dmg', promptChar: '$' },
      ],
    });
    blocks.push({
      codeLines: [
        ...
        { type: 'prompt', content: `curl -fSL -o skellycam_server "${baseUrl}/${srv}"`, promptChar: '$' },
        { type: 'prompt', content: 'chmod +x skellycam_server && xattr -cr skellycam_server', promptChar: '$' },
        { type: 'prompt', content: './skellycam_server', promptChar: '$' },
      ],
    });
  }
