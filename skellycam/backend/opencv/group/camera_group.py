...
class CameraGroup:
    def __init__(self,
        ...
        self._camera_ids = list(self._camera_configs.keys())
        logger.info(
            f"Creating camera group for cameras: {self._camera_ids} with strategy {strategy} and camera configs {self._camera_configs}"
        )
        self._event_dictionary = {}
        self._strategy_enum = strategy

        self._strategy_class = self._resolve_strategy(self._camera_ids)
    ...
    def update_camera_configs(self, camera_config_dictionary: Dict[str, CameraConfig]):
        ...
        self._camera_configs = camera_config_dictionary
