...
from skellycam.backend.controller.core_functionality.config.apply_config import apply_configuration
...
class VideoCaptureThread(threading.Thread):
    ...
    def is_opened(self) -> bool:
        ...
        return self._cv2_video_capture.isOpened()
    ...
    def _start_frame_loop(self):
        ...
        try:
            ...
            logger.error(f"Camera ID: [{self._config.camera_id}] Frame loop thread exited due to error - {e}")
            logger.exception(e)
        finally:
            logger.info(f"Camera ID: [{self._config.camera_id}] Frame capture loop has exited")
    ...
    def _get_next_frame(self):
        try:
            ...
            if success and image is not None:
                ...
                raise Exception(f"Failed to read frame from camera: {self._config.camera_id}")
        except Exception as e:
            logger.error(f"Problem when trying to read frame from Camera: {self._config.camera_id}")
            logger.exception(e)
            raise e
