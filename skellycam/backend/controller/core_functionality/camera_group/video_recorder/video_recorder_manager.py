...
class VideoRecorderManager:
    def __init__(
        ...
        video_save_directory: str = get_default_recording_folder_path(
            create_folder=False
        ),
    ):
        ...
        self._timestamp_manager = TimestampLoggerManager(
            video_save_directory=self._video_save_directory,
            camera_configs=camera_configs,
        )
        self._video_recorders: Dict[CameraId, VideoRecorder] = {
            camera_id: VideoRecorder(
                camera_config=camera_config,
                video_save_path=self._make_video_file_path(camera_id=camera_id),
            )
    ...
    def _make_video_file_path(self, camera_id: CameraId, video_format: str = "avi"):
        ...
        """
        file_name = (
            f"{Path(self._video_save_directory).stem}_camera_{camera_id}.{video_format}"
        )
