...
class VideoRecorder:
    ...
    def finish_and_close(self):
        ...
        if self.has_frames_to_save:
            raise AssertionError(
                "VideoRecorder `finish` was returned, but there are still frames to save!"
            )
        self.close()
    ...
    def close(self):
        ...
        if self.has_frames_to_save:
            raise AssertionError(
                "VideoRecorder has frames to save, but `close` was called! Theres a buggo in the logic somewhere..."
            )
...
def test_video_recording():
    ...
    save_path = (
        ...
        / "tests"
        / f"test_video_recording_{camera_id}.avi"
    )
    ...
    for frame_number in range(100):
        ...
        frame_payload = FramePayload.create(
            success=success,
