...
class TestFrameLifespanTimestamps:
    def test_initialization(self):
        ...
        timestamps = FrameLifespanTimestamps(timebase_mapping=timebase)
        ...
        assert timestamps.pre_grab_ns == 0
        assert timestamps.post_grab_ns == 0
        assert timestamps.pre_retrieve_ns == 0
        assert timestamps.post_retrieve_ns == 0
        assert timestamps.copy_to_camera_shm_ns == 0
        assert timestamps.retrieve_from_camera_shm_ns == 0
        assert timestamps.copy_to_multiframe_shm_ns == 0
        assert timestamps.retrieve_from_multiframe_shm_ns == 0
    ...
    def test_numpy_conversion(self):
        ...
        timebase = TimebaseMapping()
        original = FrameLifespanTimestamps(
            timebase_mapping=timebase,
            frame_initialized_ns=1000,
            pre_grab_ns=2000,
            post_grab_ns=3000,
        ...
        assert record_array.pre_grab_ns[0] == 2000
        assert record_array.post_grab_ns[0] == 3000
        assert record_array.pre_retrieve_ns[0] == 4000
        assert record_array.post_retrieve_ns[0] == 5000
        ...
        reconstructed = FrameLifespanTimestamps.from_numpy_record_array(record_array)
        ...
        assert reconstructed.pre_grab_ns == original.pre_grab_ns
        assert reconstructed.post_grab_ns == original.post_grab_ns
        assert reconstructed.pre_retrieve_ns == original.pre_retrieve_ns
        assert reconstructed.post_retrieve_ns == original.post_retrieve_ns
        assert reconstructed.copy_to_camera_shm_ns == original.copy_to_camera_shm_ns
        assert reconstructed.retrieve_from_camera_shm_ns == original.retrieve_from_camera_shm_ns
        assert reconstructed.copy_to_multiframe_shm_ns == original.copy_to_multiframe_shm_ns
        assert reconstructed.retrieve_from_multiframe_shm_ns == original.retrieve_from_multiframe_shm_ns
    ...
    def test_from_numpy_record_array_validation(self):
        ...
        with pytest.raises(ValueError):
            FrameLifespanTimestamps.from_numpy_record_array(wrong_array)
