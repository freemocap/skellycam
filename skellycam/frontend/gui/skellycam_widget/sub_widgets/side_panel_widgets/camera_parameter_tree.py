...
class CameraParameterTree(QWidget):
    ...
    def update_available_cameras(
        ...
        camera_str_for_debug = {
            camera_id: camera_info.description
            for camera_id, camera_info in available_cameras.items()
        }
        logger.debug(
            f"Updating available cameras in parameter tree for cameras: {camera_str_for_debug}"
        )
        self._available_cameras = available_cameras
        self._camera_configs = {
            camera_id: CameraConfig(camera_id=camera_id)
    ...
    def _update_parameter_tree(self):
        logger.debug(
            f"Updating parameter tree with new camera configs: {self._camera_configs}"
        )
        if len(self._parameter_groups) > 0:
            ...
            print("asldadasadasd")
        self._parameter_groups = {}
        for camera_config, camera_info in zip(
            self._camera_configs.values(), self._available_cameras.values()
        ):
            print(f"l;opploppl - {camera_config}")
            self._parameter_groups[
                camera_config.camera_id
            ] = self._convert_to_parameter(
                camera_config=camera_config, camera_info=camera_info
            )
            self._parameter_tree.addParameters(
                self._parameter_groups[camera_config.camera_id]
            )

    def _convert_to_parameter(
        self, camera_config: CameraConfig, camera_info: CameraDeviceInfo
    ) -> Parameter:
        ...
        camera_parameter_group = Parameter.create(
            ...
            type="group",
            children=[
                dict(name=self.tr(USE_THIS_CAMERA_STRING), type="bool", value=True),
                dict(
                    name=self.tr("Rotate"),
                    type="list",
                    limits=list(RotationTypes.__members__.keys()),
        ...
        camera_parameter_group.param(
            self.tr(USE_THIS_CAMERA_STRING)
    ...
    def _copy_settings_to_all_cameras(self, camera_id_to_copy_from: CameraId):
        ...
        for camera_id in self._camera_configs.keys():
            ...
            self._camera_configs[camera_id] = deepcopy(
                self._camera_configs[camera_id_to_copy_from]
            )
            self._camera_configs[camera_id].camera_id = camera_id
            self._camera_configs[
                camera_id
            ].use_this_camera = original_config.use_this_camera
        self._update_parameter_tree()
