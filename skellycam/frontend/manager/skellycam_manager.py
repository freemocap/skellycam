...
class SkellyCamManager:
    def __init__(
        ...
        if any(
            [
                messages_from_frontend is None,
            ...
            if not all(
                ...
                raise ValueError(
                    "If any of the backend communication objects are None, all must be None"
                )
    ...
    def handle_backend_response(self, response: BaseModel) -> None:
        ...
        if isinstance(response, CamerasDetectedResponse):
            ...
            logger.debug(
                f"Received BaseModel with no defined 'response' behavior: {response}"
            )
        else:
    ...
    def _connect_signals(self) -> None:
        self.welcome.start_session_button.clicked.connect(
            self._handle_start_session_signal
        )
    ...
    def _handle_cameras_detected_response(self, response: CamerasDetectedResponse):
        ...
        self.camera_parameter_tree.update_available_cameras(
            available_cameras=response.available_cameras
        )
