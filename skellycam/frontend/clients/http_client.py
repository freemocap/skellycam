...
class HttpClient(QObject):
    def __init__(self, hostname: str, port: int, timeout: float = 600) -> None:
        super().__init__()
        self._http_url = f"http://{hostname}:{port}"
        self._websocket_url = f"ws://{hostname}:{port}"
        self.client = httpx.Client(base_url=self._http_url, timeout=timeout)
    ...
    def connect_to_cameras(self, camera_configs: CameraConfigs, timeout: float = 60):
        ...
        request = ConnectToCamerasRequest(camera_configs=camera_configs)
