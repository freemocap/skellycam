...
class FrameWrangler:
    ...
    def _run_process(camera_group_dto: CameraGroupDTO,
        ...
        mf_payloads: deque[MultiFramePayload] = deque()
            ...
            while not camera_group_dto.ipc_flags.kill_camera_group_flag.value and not camera_group_dto.ipc_flags.global_kill_flag.value:
                ...
                if new_configs_queue.qsize() > 0:
                    if camera_group_dto.ipc_flags.record_frames_flag.value:
                        raise RuntimeError(
                            "Cannot change camera configs while recording! Ignoring new configs until recording is over...")
                    camera_configs = new_configs_queue.get()

                if camera_group_shm.new_data_available:
                    mf_payloads.append(camera_group_shm.get_next_multi_frame_payload(
                ...
                if len(mf_payloads) > 0:
                    mf_payload = mf_payloads.popleft()
                    if camera_group_dto.ipc_flags.record_frames_flag.value:
                        if not video_recorder_manager:
                            logger.info("Recording started, creating video recorder...")
                            video_recorder_manager = VideoRecorderManager.create(
                                multi_frame_payload=mf_payload,
                                camera_configs=camera_configs,
                        ...
                        if video_recorder_manager:
                            ...
                            while len(mf_payloads) > 0:
                                video_recorder_manager.add_multi_frame(mf_payloads.popleft())
