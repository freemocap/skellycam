...
def ttest_numpy_timestamp_processing():
    ...
    for camera_id in camera_ids:
        ...
        configs[camera_id] = CameraConfig(camera_id=camera_id, camera_index=camera_index).to_numpy_record_array()
        camera_index += 1
    ...
    for fr in range(num_frames):
        for camera_id in camera_ids:
            ...
            frame_time = base_time + fr * 33_333_333  # ~30fps
            jitter = np.random.randint(-1_000, 1_000)  # Add some jitter

            # Create timebase mapping
            timestamps.timebase_mapping[0] = TimebaseMapping().to_numpy_record_array()

            # Fill timestamp fields with realistic values
            timestamps.frame_initialized_ns = frame_time + jitter - 500_000
            timestamps.pre_frame_grab_ns = frame_time + jitter
            timestamps.post_frame_grab_ns = frame_time + jitter + 2_000_000
            timestamps.pre_frame_retrieve_ns = frame_time + jitter + 3_000_000
            timestamps.post_frame_retrieve_ns = frame_time + jitter + 5_000_000
            timestamps.pre_copy_to_camera_shm_ns = frame_time + jitter + 6_000_000
            timestamps.post_copy_to_camera_shm_ns = frame_time + jitter + 7_000_000
            timestamps.pre_frame_record_ns = frame_time + jitter + 8_000_000
            timestamps.post_frame_record_ns = frame_time + jitter + 10_000_000

            metadata.camera_config = configs[camera_id]
            metadata.timestamps = timestamps
            metadata.frame_number = fr
