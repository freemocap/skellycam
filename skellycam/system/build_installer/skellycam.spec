...
PACKAGE_ROOT_PATH = str(Path(sys.argv[0]).resolve().parent.parent.parent.parent)

if not (Path(PACKAGE_ROOT_PATH) / 'pyproject.toml').exists():
    raise FileNotFoundError(f"pyproject.toml not found at {PACKAGE_ROOT_PATH}")
