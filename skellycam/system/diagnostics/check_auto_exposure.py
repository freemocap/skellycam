...
def run_frame_loop(cap: cv2.VideoCapture, exposure_setting: int, auto_manual_setting: str, frames: int = 10) -> dict:
    ...
    for fr in range(frames):
        ...
        cv2.putText(annotated_image,
                    f"(Fr#{fr}) Auto/Man: {auto_manual_setting}, Set: {exposure_setting}, Actual: {cap.get(cv2.CAP_PROP_EXPOSURE)}",
                    position, font, font_scale, color, thickness)
        cv2.putText(annotated_image, f"`np.mean(image)={np.mean(image):.2f}", position2, font, font_scale, color,
                    thickness)
...
def main():
    ...
    for auto_exposure_setting, manual_exposure_setting in zip(HYPOTHETICAL_AUTO_EXPOSURE_SETTINGS,
                                                              HYPOTHETICAL_MANUAL_EXPOSURE_SETTINGS):
        print(
        ...
        for exposure_setting in EXPOSURE_SETTINGS:
            if exposure_setting == 'AUTO':
                ...
                print(
                    f"Auto Exposure after set to {auto_exposure_setting}: {cap.get(cv2.CAP_PROP_AUTO_EXPOSURE)} (hypothetically auto exposure)")
            else:
                cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, manual_exposure_setting)
                print(
                    f"Auto Exposure after set to {manual_exposure_setting}: {cap.get(cv2.CAP_PROP_AUTO_EXPOSURE)} (hypothetically manual exposure)")
...
def run_brightness_test(auto_exposure_setting: float,
    ...
    brightness_data = run_frame_loop(cap=cap, exposure_setting=exposure_setting,
                                     auto_manual_setting=str(f"({auto_exposure_setting}, {manual_exposure_setting}"))
