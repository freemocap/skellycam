...
from skellycam.opencv.group.strategies.strategies import DataSharingStrategy
...
class GroupedProcessStrategy:
    def __init__(self,
                 camera_ids: List[str],
                 data_sharing_strategy: DataSharingStrategy,
                 ):
        self._camera_ids = camera_ids
        self._processes, self._cam_id_process_map = self._create_processes(self._camera_ids, data_sharing_strategy)
    ...
    def _get_queue_size_by_camera_id(self, camera_ids: str) -> int:
        try:
            ...
            logger.error(f"Error getting queue size for camera {camera_ids}: {e}")
            return 0
    ...
    def _create_processes(
        ...
        if data_sharing_strategy == DataSharingStrategy.QUEUE:
            from skellycam.opencv.group.strategies.data_sharing_strategies.cam_group_queue_process import \
                CamGroupQueueProcess
            processes = [
                CamGroupQueueProcess(cam_id_subarray) for cam_id_subarray in camera_subarrays
            ]
        elif data_sharing_strategy == DataSharingStrategy.SHARED_MEMORY:
            from skellycam.opencv.group.strategies.data_sharing_strategies.cam_group_shared_memory_process import \
                CamGroupSharedMemoryProcess
            processes = [
                CamGroupSharedMemoryProcess(cam_id_subarray) for cam_id_subarray in camera_subarrays
            ]
        elif data_sharing_strategy == DataSharingStrategy.ZEROMQ:
            from skellycam.opencv.group.strategies.data_sharing_strategies.cam_group_zeromq_process import \
                CamGroupZeromqProcess
            processes = [
                CamGroupZeromqProcess(cam_id_subarray) for cam_id_subarray in camera_subarrays
            ]
        if data_sharing_strategy == DataSharingStrategy.PIPE:
            from skellycam.opencv.group.strategies.data_sharing_strategies.cam_group_pipe_process import \
                CamGroupPipeProcess
            processes = [
                CamGroupPipeProcess(cam_id_subarray) for cam_id_subarray in camera_subarrays
            ]
