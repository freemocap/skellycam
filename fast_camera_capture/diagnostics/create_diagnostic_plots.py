...
def create_diagnostic_plots(video_recorder_dictionary: Dict[str, VideoRecorder],
    ...
    for cam_id, video_recorder in video_recorder_dictionary.items():
        timestamps_dictionary[cam_id] = (
                video_recorder.timestamps - shared_zero_time
        )

    timestamp_diagnostics = calculate_camera_diagnostic_results(
        timestamps_dictionary
    )
    ...
    create_timestamp_diagnostic_plots(
        ...
        path_to_save_plots_png=Path(folder_to_save_plots) / "timestamp_diagnostic_plots.png",
