...
/// JPEG frame straight from the camera — no decoding.
/// Carries the compressed JPEG bytes through the pipeline.
/// Consumers decode to RGB pixels only when they actually need pixel data.
#[derive(Debug, Clone)]
pub struct FramePacket {
    /// JPEG-encoded frame bytes (~100KB for 1280×720).
    pub jpeg_bytes: Vec<u8>,
    /// Image width in pixels (known from camera metadata, not from decoding).
    pub width: u32,
